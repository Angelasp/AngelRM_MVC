...
namespace Angel.DataAccess
{
    ...
    public class BaseService : AbstractDataManager
    {
        ...
        public DataTable GetDataSourceSqlId(string tabID, string where)
        {
            try
            {
                ...
                else
                {
                    ...
                    al.Add(sql);
                    dt = SqlHelpers.Query(al).Tables[0];
                }
                return dt;
            }
            catch (Exception ex)
        ...
        public DataSet GetDataSetRunProcedure(string storedProcName, List<string> lists, string tableName)
        {
            ...
            try
            {
                IDataParameter[] iData = new SqlParameter[lists.Count];
                for (int i = 0; i < lists.Count; i++)
                {
                    iData[i] = new SqlParameter("@number" + (i + 1), lists[i]);
                }
                ds = SqlHelpers.RunProcedure(storedProcName, iData, tableName);
            }
            catch (Exception ex)
            {
                FileLog.WriteLog(ex.ToString());
        ...
        public DataSet GetDataSetRunAcquisitionApi(string storedProcName, List<string> lists, string tableName)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < lists.Count; i++)
                {
                    if (i == 0)
                    {
                        iData[i] = new SqlParameter("@datatm", lists[i]);
                    }
                    else if (i == 1)
                    {
                        iData[i] = new SqlParameter("@number3", lists[i]);
                    }
        ...
        public string[] GetOutRunProcedure(string storedProcName, string rid, string SDIId, string limit)
        {
            ...
            try
            {
                iData = new SqlParameter[6];
                iData[0] = new SqlParameter("p_rid", rid);
                iData[1] = new SqlParameter("p_SDIId", SDIId);
                iData[2] = new SqlParameter("p_limit", limit);
                iData[3] = new SqlParameter("p_better",
                SqlDbType.Int, 4, ParameterDirection.Output,
                false, 0, 0, string.Empty, DataRowVersion.Default, null);
                iData[4] = new SqlParameter("p_Worse",
                SqlDbType.Int, 4, ParameterDirection.Output,
                false, 0, 0, string.Empty, DataRowVersion.Default, null);
                iData[5] = new SqlParameter("p_equal",
                SqlDbType.Int, 4, ParameterDirection.Output,
                false, 0, 0, string.Empty, DataRowVersion.Default, null);

                iData = SqlHelpers.RunProcedureInOut(storedProcName, iData);
