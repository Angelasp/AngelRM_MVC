	...
	return this.each( function() {
		...
		if( ts.p.toolbar[0] ) {
			...
			if(ts.p.toolbar[1]==="both") {
				...
				$(grid.ubDiv).addClass( tbstyle + " ui-userdata-bottom").attr("id","Angel_"+this.id).insertAfter(grid.hDiv).width(grid.width);
...
$.jgrid.extend({
	...
	viewGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			frmgr_id = "ViewGrid_"+gID, frmAngel_id = "ViewTbl_"+gID,
			...
			tbl =$("<table id='"+frmAngel_id+"' class='EditTable ViewTable'><tbody></tbody></table>");
			$($t.p.colModel).each( function() {
				var fmto = this.formoptions;
				maxCols = Math.max(maxCols, fmto ? fmto.colpos || 0 : 0 );
				maxRows = Math.max(maxRows, fmto ? fmto.rowpos || 0 : 0 );
